...
- **`radar.js`** fetches "real" signals from public, keyless APIs in parallel (`Promise.allSettled`): Hacker News (Algolia) and Reddit JSON only (no job-board sources — those were tried and removed). Each source fails independently; results are deduped, capped per category (max 14), and categorized via `RADAR_CATS` (modelos / ferramentas / mercado / homeassistant / negocios).
